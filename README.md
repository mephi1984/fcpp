...
Для того, чтобы программа «Индексатор» работала достаточно быстро, вам следует распараллелить обработку файлов между несколькими потоками. Создайте пул потоков, и распределите задачи по обработке файлов между потоками. Таким образом вы можете обрабатывать несколько файлов одновременно. Обеспечите синхронизацию данных, вы должны написать код так, чтобы избежать состояний, таких как deadlock или состояние гонки.
...
В случае если при работе программы возникла ошибка, на экране также отобразите сообщение о внутренней ошибке.
...
Настройки базы данных (хост, порт, название БД, имя пользователя, пароль) храните в ini-файле конфигурации.
...
Все настройки программ храните в ini-файле. Ваша программа должна парсить этот файл при запуске.

В этом файле храните следующие параметры:
...
3. Выложите проект на GitHub в виде исходного кода и вашего ini-файла конфигурации
